"""Configuration for Pydantic schemas."""
...
class ConfigDict(TypedDict, total=False):
    ...
    * `ignore` - Ignore any extra attributes.
    ...
    frozen: bool
    """
    Whether schemas are faux-immutable, i.e. whether `__setattr__` is allowed, and also generates
    ...
    """
    Whether to populate schemas with the `value` property of enums, rather than the raw enum.
    ...
    """
    Whether to build schemas and look up discriminators of tagged unions using python object attributes.
    """
    ...
    """
    # whether instances of schemas and dataclasses (including subclass instances) should re-validate, default 'never'
    revalidate_instances: Literal['always', 'never', 'subclass-instances']
    """
    When and how to revalidate schemas and dataclasses during validation. Accepts the string
    values of `'never'`, `'always'` and `'subclass-instances'`. Defaults to `'never'`.

    - `'never'` will not revalidate schemas and dataclasses during validation
    - `'always'` will revalidate schemas and dataclasses during validation
    - `'subclass-instances'` will revalidate schemas and dataclasses during validation if the instance is a
    ...
    This can be useful to avoid the overhead of building schemas which are only
    used nested within other schemas, or when you want to manually define type namespace via
    ...
    !!! note
        `defer_build` does not work by default with FastAPI Pydantic schemas. By default, the validator and serializer
        for said schemas is constructed immediately for FastAPI routes. You also need to define
        [`experimental_defer_build_mode=('model', 'type_adapter')`][pydantic.config.ConfigDict.experimental_defer_build_mode] with FastAPI
        schemas in order for `defer_build=True` to take effect. This additional (experimental) parameter is required for
